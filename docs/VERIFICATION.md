...
The v8 ending uses a two-second icon gesture from 57.5 to 59.5: 0.3 seconds of growth, 1.4 seconds for two left-right cycles, and 0.3 seconds returning to the exact normal icon. The maximum enlargement is 18%. It holds the normal icon before and after the gesture. Source frame checks and actual Chrome playback confirmed the left-right-left-right sequence, visible growth, and return to normal size. The preceding 57 seconds of normalized H.264 video and the entire audio stream match v7. All 1,800 frames decode, desktop/mobile playback passes, and all 28 deployed assets match the tested build.
